...
alter view Relation_between_Goals_Annual_Salary
as
...
declare @salary money = 10000000, @goals INT = 10
  IF EXISTS ( SELECT * FROM Performance_Player
	...
	END
END
EXEC  Player_Check @team_name= 'arsenal'
...
declare @salary money = 10000000, @goals INT = 10
  IF EXISTS ( SELECT * FROM Performance_Player
	...
	BEGIN
		...
		from Performance_Player 
		where Annual_Salary < 10000000 and goals >10 and @team_name=team_name
		group by full_name , Team_name,Annual_Salary , Goals, position
	END
	Else
	BEGIN
	...
	END
END
EXEC  Player_has_chance @team_name= 'arsenal'
